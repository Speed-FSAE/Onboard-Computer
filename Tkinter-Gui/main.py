...
# Titles
titleLabel = Label(root, text="Driver Display V1", bg=globalBG, fg="red",
                   font="900px").place(x=(screenX*0.45), y=(screenY*0.9))
...
def revDown():
    global rpms
    rpms -= randrange(850, 1000)
    revCounter.config(text=str(rpms), font="22")
    gearNumber()
    return rpms
...
# revButton.bind('<ButtonPress-1>', lambda event: revUp())
# revButton.bind('<ButtonRelease-1>', lambda event: revDown())

revCounter = Label(root, text=str(rpms), bg=globalBG, fg="white", font="22")
...
gearIndicator = Label(root, bg=globalBG, text=str(gear),
                      fg="white", font=("Arial", int(screenAvg*0.25)))
